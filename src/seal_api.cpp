...
namespace rhombus
{
    ...
    void gen_aux_params(AuxParms &aux_parms, uint32_t mod_bits, const std::vector<seal::Modulus> &coeff_mods)
    {
        if (mod_bits < 1 || mod_bits > 64)
            throw std::invalid_argument("the modulus bits should be in [1, 64]");
        uint32_t kNumMods = coeff_mods.size();
        aux_parms.pow_2_t_mod_qi_.resize(kNumMods - 1);
        aux_parms.inv_2_t_mod_qi_.resize(kNumMods - 1);
        aux_parms.inv_qi_mod_2_t_.resize(kNumMods - 1);
        aux_parms.pow_2_64_mod_qi_.resize(kNumMods - 1);
        ...
        if (mod_bits == 64){
            for (size_t i = 0; i < kNumMods - 1; ++i)
                aux_parms.pow_2_t_mod_qi_[i] = seal::util::multiply_uint_mod((1ULL << 32), (1ULL << 32), coeff_mods[i]);
            uint64_t pow_2_t[]{0, 1};
            for (size_t i = 0; i < kNumMods - 1; ++i){
                aux_parms.pow_2_64_mod_qi_[i] = seal::util::multiply_uint_mod((1ULL << 32), (1ULL << 32), coeff_mods[i]);
                seal::util::try_invert_uint_mod(aux_parms.pow_2_t_mod_qi_[i], coeff_mods[i], aux_parms.inv_2_t_mod_qi_[i]);
                uint64_t vec_qi[]{coeff_mods[i].value(), 0};
                uint64_t vec_inv_qi_mod_2_t[]{0, 0};
            ...
            uint64_t pow_2_t{1ULL << mod_bits};
            for (size_t i = 0; i < kNumMods - 1; ++i)
                aux_parms.pow_2_t_mod_qi_[i] = seal::util::barrett_reduce_64(pow_2_t, coeff_mods[i]);

            for (size_t i = 0; i < kNumMods - 1; ++i){
                aux_parms.pow_2_64_mod_qi_[i] = seal::util::multiply_uint_mod((1ULL << 32), (1ULL << 32), coeff_mods[i]);
                seal::util::try_invert_uint_mod(aux_parms.pow_2_t_mod_qi_[i], coeff_mods[i], aux_parms.inv_2_t_mod_qi_[i]);
                seal::util::try_invert_uint_mod(coeff_mods[i].value(), pow_2_t, aux_parms.inv_qi_mod_2_t_[i]);
            }
        }

        if (kNumMods >= 3)
            seal::util::try_invert_uint_mod(coeff_mods[1].value(), coeff_mods[0], aux_parms.inv_q1_mod_q0_);
        if (kNumMods == 4){
            seal::util::try_invert_uint_mod(coeff_mods[2].value(), coeff_mods[0], aux_parms.inv_q2_mod_q0_);
            seal::util::try_invert_uint_mod(coeff_mods[2].value(), coeff_mods[1], aux_parms.inv_q2_mod_q1_);
        }
    }
    ...
    {
        auto context_data_ptr = context.get_context_data(rlwe_ctxts[0].parms_id());
        uint32_t rlwe_num = rlwe_ctxts.size();
        uint32_t N = rlwe_ctxts[0].poly_modulus_degree();
        if (rlwe_num * (1 << h) > N)
        ...
        uint32_t log_pad_rlwe_num = seal::util::get_significant_bit_count(rlwe_num - 1);
        ...
        for (uint32_t i = 0; i < log_pad_rlwe_num; ++i)
        {
            ...
            for (uint32_t j = 0; j < steps_j; j += thread_count)
            {
                ...
                thread_count = (step_last < threads) ? step_last : threads;
                ...
                for (size_t k = 0; k < thread_count; ++k){
                    if (i == 0){
                        ...
                        bool is_zero = (rlwe2_index >= rlwe_num);
                        if (is_zero){
                ...
                const seal::GaloisKeys &galois_keys, const seal::SEALContext &context, uint32_t nthreads)
    {
        ...
        if (ell > h)
            throw std::invalid_argument("require: ell <= h");
        if (h > logN)
            throw std::invalid_argument("require: h <= logN");
        ...
        // d1 = ct + tau_{N/2^j + 1}(ct), d2 = X^{-2^j}(ct - tau_{N/2^j + 1})
        auto expand_to_two_rlwes = [&](const seal::Ciphertext &input_ct, seal::Ciphertext &dest1,
                                       seal::Ciphertext &dest2, uint32_t j){
            seal::Ciphertext temp0, temp1;
            temp0 = input_ct;
        ...
        for (uint32_t j = 0; j < ell; ++j){ // for each level
            for (uint32_t k = 0; k < (1UL << j); k += nthreads){ // for each ciphertext in current level
                uint32_t step_last = (1UL << j) - k;
                uint32_t thread_count = (step_last < nthreads) ? step_last : nthreads;
                std::vector<std::thread> threads_pool(thread_count);
                for (uint32_t p = 0; p < thread_count; ++p){
                    threads_pool[p] = std::thread(expand_to_two_rlwes, std::cref(destination[k + p]),
                                                  std::ref(destination[k + p]), std::ref(destination[(k + p) + (1UL << j)]), j + logN - h);
                }
                std::for_each(threads_pool.begin(), threads_pool.end(), [](std::thread &t){t.join();});
            }
        }
    ...
    void Expand(const seal::Ciphertext &ctxt, std::vector<seal::Ciphertext> &destination, uint32_t ell, uint32_t h, uint64_t expand_factor,
                const seal::GaloisKeys &galois_keys, const seal::SEALContext &context, uint32_t nthreads, bool to_ntt)
    {
        ...
        // expand: the output ciphertexts are in INTT form
        expand(dup_ctxt, destination, ell, h + ell, galois_keys, context, nthreads);

        if (to_ntt)
        {
            transform_batched_to_ntt_inplace(destination, context, nthreads);
        }
    }
    ...
    void drop_unrelated_coeffs(seal::Ciphertext &ctxt, uint64_t vec_size, const seal::SEALContext &context)
    {
        ...
        uint64_t vec_size_extend = 1ULL << get_significant_bit_count(vec_size - 1);
    ...
    void generate_random_plaintext(seal::Plaintext &rand_plaintext, const seal::SEALContext &context, seal::parms_id_type parms_id, bool is_ntt_form)
    {
        auto &parms = context.get_context_data(parms_id)->parms();
        auto &coeff_modulus = parms.coeff_modulus();
        auto coeff_modulus_size = coeff_modulus.size();
        auto poly_mod_degree = parms.poly_modulus_degree();


        std::mt19937_64 gen(std::random_device{}());
        rand_plaintext.parms_id() = seal::parms_id_zero;
        rand_plaintext.resize(poly_mod_degree * coeff_modulus_size);
        for (size_t k = 0; k < coeff_modulus_size; ++k){
            std::uniform_int_distribution<uint64_t> dist(0, coeff_modulus[k].value() - 1);
            std::generate_n(rand_plaintext.data() + k * poly_mod_degree, poly_mod_degree,
                            [&](){return dist(gen);});
        }
        
        rand_plaintext.scale() = 1.;
        if (is_ntt_form){
            rand_plaintext.parms_id() = parms_id;
        }
    }
