...
namespace rhombus {
    ...
    template <typename T, typename U, typename std::enable_if<std::is_integral_v<T> && std::is_integral_v<U>, int>::type = 0>
                                          ...
                                          size_t nrows, size_t ncols, uint64_t &max_diff, uint32_t mod_bits)
    {
        ...
        return std::move(std::make_tuple(max_diff_row_index, max_diff_col_index));
    }
    ...
    // substitute over coefficients mod X^N+1.
    // Input: coefficient vector of a(X)
